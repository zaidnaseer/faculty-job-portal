...
const HRApplicants = () => {
    ...
    return (
        <div className="container py-8">
            <h1 className="text-3xl font-bold mb-8">HR Applicants</h1>
                ...
                {showFilters && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t">
                        <div>
                            ...
                            <select name="job" value={filters.job} onChange={handleFilterChange} className="form-input">
                                {jobs.map(job => (
                                    <option key={job} value={job}>{job === 'all' ? 'All Positions' : job}</option>
                                ))}
                            </select>
                        </div>
            ...
            {filteredApplicants.length > 0 ? (
                ...
                <div className="bg-white rounded-lg shadow-md p-8 text-center">
                    <h3 className="text-xl font-medium text-gray-700 mb-2">No matching applicants found</h3>
